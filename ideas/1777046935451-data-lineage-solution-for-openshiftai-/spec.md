...
Data scientists and ML engineers working in OpenShiftAI currently lack visibility into how data flows through their ML pipelines. This creates several critical challenges:

1. **Reproducibility**: Unable to trace back which exact data sources and transformations produced a specific model
2. **Debugging**: When model performance degrades, difficult to identify which upstream data changes caused the issue
3. **Compliance**: Cannot demonstrate data governance and lineage for regulatory requirements (GDPR, healthcare, financial)
4. **Collaboration**: Team members struggle to understand dependencies between datasets, features, and models
5. **Trust**: Stakeholders lack confidence in model outputs without visibility into data provenance

Without automated lineage tracking, teams resort to manual documentation which is error-prone, quickly becomes outdated, and doesn't scale as pipelines grow in complexity.
...
1. **Lineage Capture Granularity**: Should we track lineage at the row/column level (e.g., "feature X derived from columns A, B in dataset Y") or only at the dataset/model level? Row-level provides more insight but generates significantly more metadata.

2. **Cross-Cluster Lineage**: How do we handle lineage when data flows between different OpenShift clusters or external systems (e.g., data ingested from on-prem database, processed in cloud OpenShiftAI, model deployed to edge)? Do we need federated lineage tracking?

3. **Lineage Retention Policy**: How long should historical lineage be retained? Options: retain forever (expensive), retain for compliance period (e.g., 7 years for regulated industries), or retain based on model lifecycle (delete when model is retired)?
